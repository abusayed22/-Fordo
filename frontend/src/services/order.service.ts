...
} from "@/zodValidation/order.validation";
...
export async function createOrderAction(
  ...
  if (!parsedPayload.success) {
    return {
      success: false,
      message:
        parsedPayload.error.issues[0]?.message || "Invalid order details",
      data: null,
    };
  }

  try {
    const response = await httpClient.post<ICreateOrderResponseData>("/order", parsedPayload.data);
...
export async function getOrdersData(params: OrdersQueryParams) {
  ...
  const res = await httpClient.get<OrdersListResponse>(
...
export async function updateOrderStatusAction(payload: {
  ...
  const res = await httpClient.patch(`/orders/${payload.orderId}/status`, {
