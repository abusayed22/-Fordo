...
const SaleBarChart = ({data}: SaleBarChartProps) => {

    if(!data || !Array.isArray(data)){
        return (
            <Card className="col-span-4">
                <CardHeader>
                    <CardTitle>Sale Trends</CardTitle>
                    <CardDescription>Monthly Sale Statistics</CardDescription>
                </CardHeader>
    ...
    const formattedData = data.map((item) => ({
        ...
        sales : Number(item.count)
    }))


    if(!formattedData.length || formattedData.every(item => item.sales === 0)){
        return (
          <Card className="col-span-4">
            <CardHeader>
              <CardTitle>Sale Trends</CardTitle>
